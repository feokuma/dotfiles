...
PanelWindow {
    ...
    // No exclusive keyboard focus: it would make Hyprland route all pointer
    // events here (see PopupBase). Esc closing is handled by the global
    // non-consuming bind calling the shell's IPC ("popup" target).
